...
v11 的“输入漂移”现已完成版本化根因纠正，但 v11 失败事实不回写。原始 2026-09-09 冻结命令已从封存会话恢复：它使用未显式设置 `LC_ALL=C` 的 GNU `sort -z`，因此继承主机 locale；2026-09-23 同一 A 端主机回读为 `en_US.UTF-8`。用原命令只读复算，s9234、s38584、wb_dma 三个摘要和数量全部仍与冻结值一致；再用 `LC_ALL=C sort -z` 复算，s9234 与 wb_dma 恰好得到 v11 Python `sorted()` 报出的所谓“当前摘要”，s38584 在两种顺序下相同。因此没有证据证明文件内容或路径集合发生漂移，v11 是排序语义不一致造成的假阳性。`contracts/blind_inventory_ordering_v2.json` 与 `data/manifests/blind_input_inventory_freeze_v2.json` 同时冻结历史 locale 顺序和 bytewise 顺序；`data/manifests/blind_inventory_drift_provenance_v1.json` 保留根因链。v11 作业仍禁止重试、重排队或重跑，P0 仍未解锁；任何新执行必须进入 v12，新设计同时验证两种摘要，经独立复核并获得新授权后方可进行，训练继续禁止。
