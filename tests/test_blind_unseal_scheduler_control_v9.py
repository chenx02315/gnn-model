...
BUNDLE = "/temp/jiangchuanc/blind_runtime_unseal_v9_bundle"
ARGV = ["python3", BUNDLE + "/src/data/run_blind_unseal_v9.py", "--bundle-root", BUNDLE]
sys.path.insert(0, str(ROOT / "src" / "data"))

from validate_blind_unseal_registration_v9 import ValidationError, sha256_bytes, validate_registration
from validate_blind_unseal_scheduler_audit_v9 import validate_scheduler_audit
...
def write_artifacts(directory, contract, contract_sha, tool):
    ...
    receipt = {"schema_version": "blind-runtime-unseal-receipt-v9", "status": "PASS", "formal_runtime_membership_sha256": contract["scope"]["formal_runtime_membership_sha256"], "method_registry_sha256": contract["scope"]["method_registry_sha256"], "tool_set_sha256": tool, "contract_sha256": contract_sha, "circuits": rows, "failure_code": None}
    receipt_bytes = json.dumps(receipt, sort_keys=True).encode("utf-8")
    receipt_sha = sha256_bytes(receipt_bytes)
    files = {"CONSUMED": json.dumps({"schema_version": "blind-runtime-unseal-consumed-v9", "status": "CONSUMED", "contract_sha256": contract_sha, "tool_set_sha256": tool}).encode("utf-8"), "receipt.json": receipt_bytes, "receipt.json.sha256": (receipt_sha + "  receipt.json\n").encode("ascii"), "RELEASED": json.dumps({"schema_version": "blind-runtime-unseal-release-v9", "status": "RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING", "contract_sha256": contract_sha, "receipt_sha256": receipt_sha}).encode("utf-8")}
...
def audit(raw_registration, bjobs, global_raw, bhist, bacct, stdout, stderr, artifacts):
    return {"schema_version": "blind-runtime-unseal-scheduler-audit-v9", "status": "RELEASED_VERIFIED", "job_id": "12345", "registration_receipt_sha256": sha256_bytes(raw_registration), "protocol_sha256": digest("protocol"), "raw_bjobs_capture_sha256": sha256_bytes(bjobs), "raw_global_bjobs_capture_sha256": sha256_bytes(global_raw), "bhist_raw_capture_sha256": sha256_bytes(bhist), "bacct_raw_capture_sha256": sha256_bytes(bacct), "stdout_capture_sha256": sha256_bytes(stdout), "stderr_capture_sha256": sha256_bytes(stderr), "four_artifact_sha256": artifacts}


class SchedulerControlv9Test(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.blob_check = mock.patch("validate_blind_unseal_registration_v9._git_blob_equals_worktree")
        self.blob_check.start()
        self.staging_check = mock.patch("validate_blind_unseal_registration_v9.verify_staged_bundle", return_value="synthetic-staging-verified")
        ...
        self.runner_path = ROOT / "src" / "data" / "run_blind_unseal_v9.py"
        self.contract_path = self.directory / "implementation-contract.json"
        runner_sha = sha256_bytes(self.runner_path.read_bytes())
        self.contract = json.loads((ROOT / "contracts" / "blind_runtime_unseal_v9.json").read_text(encoding="utf-8"))
        self.contract["status"] = "FINAL_HELD_UNSEAL_REVIEW_REQUIRED"
        self.contract["job_template"] = {"path": "contracts/blind_runtime_unseal_job_v9_template.json", "sha256": sha256_bytes(self.template_path.read_bytes())}
        ...
        self.contract["implementation"]["artifact_sha256"] = {"src/data/run_blind_unseal_v9.py": runner_sha}
        ...
        self.raw_al = bjobs_al(); self.manifest = b'{"schema_version":"blind-runtime-unseal-control-manifest-v9"}\n'
        ...
        self.tool = sha256_bytes(("src/data/run_blind_unseal_v9.py:" + runner_sha + "\n").encode("utf-8"))
    ...
    def test_v9_absolute_cwd_and_long_capture_are_mandatory(self):
    ...
    def test_v9_external_manifest_digest_is_bound(self):
        with self.assertRaises(ValidationError):
