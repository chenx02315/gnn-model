...
from validate_blind_unseal_scheduler_audit_v10 import validate_scheduler_audit
...
def registration(raw, global_raw, template_bytes, contract_bytes, raw_al=None, manifest=None):
    ...
    manifest = b'{"schema_version":"blind-runtime-unseal-control-manifest-v10"}\n' if manifest is None else manifest
    argv = job_template()["registration"]["command_argv"]
    return {"schema_version": "blind-runtime-unseal-registration-v10", "status": "REGISTERED_HELD", "job_id": "12345", "owner": "jiangchuanc", "submission_host": "host-a", "submit_time_raw": "Wed Sep 10 07:07:59", "reviewed_commit": "a" * 40, "contract_sha256": sha256_bytes(contract_bytes), "protocol_sha256": digest("protocol"), "gate_snapshot_sha256": digest("gate"), "runner_sha256": sha256_bytes((ROOT / "src" / "data" / "run_blind_unseal_v10.py").read_bytes()), "job_template_sha256": sha256_bytes(template_bytes), "normalized_argv": argv, "normalized_argv_sha256": hashlib.sha256(json.dumps(argv, separators=(",", ":")).encode("utf-8")).hexdigest(), "queue": "normal", "resource_request": "select[type==X]", "cwd": BUNDLE, "stdout_path": "/evidence/stdout.log", "stderr_path": "/evidence/stderr.log", "raw_bjobs_capture_sha256": sha256_bytes(raw), "raw_global_bjobs_capture_sha256": sha256_bytes(global_raw), "raw_bjobs_al_capture_sha256": sha256_bytes(raw_al), "external_control_manifest_path": "/temp/jiangchuanc/blind_runtime_unseal_v10_control_manifest.json", "external_control_manifest_sha256": sha256_bytes(manifest), "specified_cwd": BUNDLE}
...
def write_artifacts(directory, contract, contract_sha, tool):
    ...
    receipt = {"schema_version": "blind-runtime-unseal-receipt-v10", "status": "PASS", "formal_runtime_membership_sha256": contract["scope"]["formal_runtime_membership_sha256"], "method_registry_sha256": contract["scope"]["method_registry_sha256"], "tool_set_sha256": tool, "contract_sha256": contract_sha, "circuits": rows, "failure_code": None}
    receipt_bytes = json.dumps(receipt, sort_keys=True).encode("utf-8")
    receipt_sha = sha256_bytes(receipt_bytes)
    files = {"CONSUMED": json.dumps({"schema_version": "blind-runtime-unseal-consumed-v10", "status": "CONSUMED", "contract_sha256": contract_sha, "tool_set_sha256": tool}).encode("utf-8"), "receipt.json": receipt_bytes, "receipt.json.sha256": (receipt_sha + "  receipt.json\n").encode("ascii"), "RELEASED": json.dumps({"schema_version": "blind-runtime-unseal-release-v10", "status": "RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING", "contract_sha256": contract_sha, "receipt_sha256": receipt_sha}).encode("utf-8")}
...
def audit(raw_registration, bjobs, global_raw, bhist, bacct, stdout, stderr, artifacts):
    return {"schema_version": "blind-runtime-unseal-scheduler-audit-v10", "status": "RELEASED_VERIFIED", "job_id": "12345", "registration_receipt_sha256": sha256_bytes(raw_registration), "protocol_sha256": digest("protocol"), "raw_bjobs_capture_sha256": sha256_bytes(bjobs), "raw_global_bjobs_capture_sha256": sha256_bytes(global_raw), "bhist_raw_capture_sha256": sha256_bytes(bhist), "bacct_raw_capture_sha256": sha256_bytes(bacct), "stdout_capture_sha256": sha256_bytes(stdout), "stderr_capture_sha256": sha256_bytes(stderr), "four_artifact_sha256": artifacts}


class SchedulerControlv10Test(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.blob_check = mock.patch("validate_blind_unseal_registration_v10._git_blob_equals_worktree")
        self.blob_check.start()
        self.staging_check = mock.patch("validate_blind_unseal_registration_v10.verify_staged_bundle", return_value="synthetic-staging-verified")
        ...
        self.contract["job_template"] = {"path": "contracts/blind_runtime_unseal_job_v10_template.json", "sha256": sha256_bytes(self.template_path.read_bytes())}
        ...
        self.raw_al = bjobs_al(); self.manifest = b'{"schema_version":"blind-runtime-unseal-control-manifest-v10"}\n'
        ...
        self.tool = sha256_bytes(("src/data/run_blind_unseal_v10.py:" + runner_sha + "\n").encode("utf-8"))
    ...
    def test_v10_absolute_cwd_and_long_capture_are_mandatory(self):
    ...
    def test_v10_external_manifest_digest_is_bound(self):
        with self.assertRaises(ValidationError):
