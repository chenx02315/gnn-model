...
import validate_blind_unseal_registration_v10 as V10


class BlindUnsealStagingV10Test(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.repo = pathlib.Path(self.temp.name) / "bundle"
        self.repo.mkdir()
        for relative in V10.CONTROL_MANIFEST_PATHS:
    ...
    def manifest(self, mutate=None):
        ...
        value = {"schema_version": "blind-runtime-unseal-control-manifest-v10",
    ...
    def verify(self, manifest=None, freeze=None, root=None):
        ...
        with mock.patch.object(V10, "EXPECTED_BUNDLE_ROOT", root), \
             mock.patch.object(V10, "EXPECTED_EXTERNAL_CONTROL_MANIFEST", str(external)):
            return V10.verify_staged_bundle(root, self.freeze if freeze is None else freeze,
                                           raw)
    ...
    def test_dirty_checkout_and_digest_drift_fail(self):
        target = self.repo / pathlib.PurePosixPath(V10.CONTROL_MANIFEST_PATHS[0])
        target.write_bytes(target.read_bytes() + b"drift")
        with self.assertRaises(V10.ValidationError):
            self.verify()
        subprocess.check_call(("git", "checkout", "--", V10.CONTROL_MANIFEST_PATHS[0]), cwd=str(self.repo))
    ...
    def test_manifest_argument_must_equal_fixed_external_file(self):
        ...
        with mock.patch.object(V10, "EXPECTED_BUNDLE_ROOT", str(self.repo)), \
             mock.patch.object(V10, "EXPECTED_EXTERNAL_CONTROL_MANIFEST", str(external)):
            with self.assertRaises(V10.ValidationError):
                V10.verify_staged_bundle(str(self.repo), self.freeze, raw)
