...
from build_blind_unseal_lifecycle_v9 import (LifecycleError, build_final_job_spec,
...
class BlindUnsealLifecyclev9Test(unittest.TestCase):
    def setUp(self):
        materialized_contract = json.loads((ROOT / "contracts" / "blind_runtime_unseal_v9.json").read_text())
        freeze_commit = materialized_contract.get("lifecycle", {}).get("implementation_freeze_commit")
        if freeze_commit:
            self.draft_template = json.loads(subprocess.check_output(("git", "show", freeze_commit + ":contracts/blind_runtime_unseal_job_v9_template.json"), cwd=str(ROOT)).decode())
            self.draft_contract = json.loads(subprocess.check_output(("git", "show", freeze_commit + ":contracts/blind_runtime_unseal_v9.json"), cwd=str(ROOT)).decode())
        else:
            self.draft_template = json.loads((ROOT / "contracts" / "blind_runtime_unseal_job_v9_template.json").read_text())
        ...
        for name in ("build_blind_unseal_lifecycle_v9.py", "run_blind_unseal_v9.py",
                     "validate_blind_unseal_registration_v9.py", "validate_blind_unseal_scheduler_audit_v9.py"):
            (self.repo / "src" / "data" / name).write_bytes((ROOT / "src" / "data" / name).read_bytes())
        self.runner = self.repo / "src" / "data" / "run_blind_unseal_v9.py"
        self.external_manifest_path = str(self.repo.parent / (self.repo.name + "-control-manifest.json"))
        self.constant_patches = [
            mock.patch("build_blind_unseal_lifecycle_v9.EXPECTED_EXTERNAL_CONTROL_MANIFEST", self.external_manifest_path),
            mock.patch("validate_blind_unseal_registration_v9.EXPECTED_EXTERNAL_CONTROL_MANIFEST", self.external_manifest_path),
            mock.patch("validate_blind_unseal_registration_v9.verify_staged_bundle", return_value="synthetic-staging-verified"),
        ]
        ...
        self.artifacts = {"src/data/run_blind_unseal_v9.py": sha256_bytes(self.runner.read_bytes())}
    ...
    def _bundle(self):
        ...
        manifest = {"schema_version": "blind-runtime-unseal-control-manifest-v9",
        ...
        (self.repo / "contracts" / "blind_runtime_unseal_job_v9_template.json").write_bytes(tb)
        (self.repo / "contracts" / "blind_runtime_unseal_v9.json").write_bytes(cb)
        ...
        (self.repo / "contracts" / "blind_runtime_unseal_registration_v9.json").write_bytes(rb)
        job = build_final_job_spec(rb, tb, cb, str(self.repo / "out")); jb = canonical_bytes(job)
        (self.repo / "contracts" / "blind_runtime_unseal_job_v9.json").write_bytes(jb)
        review = build_independent_review(rb, jb, cb, self.raw, self.global_raw, self.raw_al, self.manifest, "812345",
          str(self.repo / "contracts" / "blind_runtime_unseal_job_v9_template.json"),
          str(self.repo / "contracts" / "blind_runtime_unseal_v9.json"), str(self.runner))
        vb = canonical_bytes(review); (self.repo / "contracts" / "blind_unseal_independent_review_v9.json").write_bytes(vb)
    ...
    def test_synthetic_e2e_calls_runner_validate_bundle(self):
        ...
        spec = importlib.util.spec_from_file_location("synthetic_v9_runner", str(self.runner))
    ...
    def test_bad_raw_or_fake_review_cannot_pass(self):
        ...
        with self.assertRaises(LifecycleError):
            build_independent_review(rb, jb, cb, self.raw.replace(b"PSUSP", b"RUN"), self.global_raw, self.raw_al, self.manifest, "812345",
              str(self.repo / "contracts" / "blind_runtime_unseal_job_v9_template.json"), str(self.repo / "contracts" / "blind_runtime_unseal_v9.json"), str(self.runner))
