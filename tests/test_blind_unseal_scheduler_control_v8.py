...
from validate_blind_unseal_registration_v8 import ValidationError, sha256_bytes, validate_registration
from validate_blind_unseal_scheduler_audit_v8 import validate_scheduler_audit
...
def bjobs_row(job_id="12345", state="PSUSP", command=None, owner="jiangchuanc", queue="normal", cwd="/bundle"):
    ...
    command = command or "python3 src/data/run_blind_unseal_v8.py --bundle-root ."
...
def job_template():
    return {"schema_version": "blind-runtime-unseal-job-v8", "status": "FINAL_HELD_JOB_TEMPLATE", "registration": {"job_id": "12345", "git_commit": "a" * 40, "initial_scheduler_state": "PSUSP", "array_allowed": False, "retry_allowed": False, "requeue_allowed": False, "rerun_allowed": False, "command_argv": ["python3", "src/data/run_blind_unseal_v8.py", "--bundle-root", "."], "queue": "normal", "resource_request": "select[type==X]", "cwd": "/bundle", "stdout_path": "/evidence/stdout.log", "stderr_path": "/evidence/stderr.log"}, "measurement_files": ["01_single_boundaries/measurements.tsv"], "circuits": [{"name": "s9234", "root": "/blind/s9234"}], "template_rule": "synthetic final template", "lifecycle": {"implementation_freeze_commit": "a" * 40, "held_job_id": "12345", "finalization_event": "bsub_H_then_raw_bjobs_capture"}}


def registration(raw, global_raw, template_bytes, contract_bytes):
    argv = job_template()["registration"]["command_argv"]
    return {"schema_version": "blind-runtime-unseal-registration-v8", "status": "REGISTERED_HELD", "job_id": "12345", "owner": "jiangchuanc", "submission_host": "host-a", "submit_time_raw": "Wed Sep 10 07:07:59", "reviewed_commit": "a" * 40, "contract_sha256": sha256_bytes(contract_bytes), "protocol_sha256": digest("protocol"), "gate_snapshot_sha256": digest("gate"), "runner_sha256": sha256_bytes((ROOT / "src" / "data" / "run_blind_unseal_v8.py").read_bytes()), "job_template_sha256": sha256_bytes(template_bytes), "normalized_argv": argv, "normalized_argv_sha256": hashlib.sha256(json.dumps(argv, separators=(",", ":")).encode("utf-8")).hexdigest(), "queue": "normal", "resource_request": "select[type==X]", "cwd": "/bundle", "stdout_path": "/evidence/stdout.log", "stderr_path": "/evidence/stderr.log", "raw_bjobs_capture_sha256": sha256_bytes(raw), "raw_global_bjobs_capture_sha256": sha256_bytes(global_raw)}
...
def write_artifacts(directory, contract, contract_sha, tool):
    ...
    receipt = {"schema_version": "blind-runtime-unseal-receipt-v8", "status": "PASS", "formal_runtime_membership_sha256": contract["scope"]["formal_runtime_membership_sha256"], "method_registry_sha256": contract["scope"]["method_registry_sha256"], "tool_set_sha256": tool, "contract_sha256": contract_sha, "circuits": rows, "failure_code": None}
    receipt_bytes = json.dumps(receipt, sort_keys=True).encode("utf-8")
    receipt_sha = sha256_bytes(receipt_bytes)
    files = {"CONSUMED": json.dumps({"schema_version": "blind-runtime-unseal-consumed-v8", "status": "CONSUMED", "contract_sha256": contract_sha, "tool_set_sha256": tool}).encode("utf-8"), "receipt.json": receipt_bytes, "receipt.json.sha256": (receipt_sha + "  receipt.json\n").encode("ascii"), "RELEASED": json.dumps({"schema_version": "blind-runtime-unseal-release-v8", "status": "RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING", "contract_sha256": contract_sha, "receipt_sha256": receipt_sha}).encode("utf-8")}
...
def audit(raw_registration, bjobs, global_raw, bhist, bacct, stdout, stderr, artifacts):
    return {"schema_version": "blind-runtime-unseal-scheduler-audit-v8", "status": "RELEASED_VERIFIED", "job_id": "12345", "registration_receipt_sha256": sha256_bytes(raw_registration), "protocol_sha256": digest("protocol"), "raw_bjobs_capture_sha256": sha256_bytes(bjobs), "raw_global_bjobs_capture_sha256": sha256_bytes(global_raw), "bhist_raw_capture_sha256": sha256_bytes(bhist), "bacct_raw_capture_sha256": sha256_bytes(bacct), "stdout_capture_sha256": sha256_bytes(stdout), "stderr_capture_sha256": sha256_bytes(stderr), "four_artifact_sha256": artifacts}


class SchedulerControlV8Test(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.blob_check = mock.patch("validate_blind_unseal_registration_v8._git_blob_equals_worktree")
        ...
        self.runner_path = ROOT / "src" / "data" / "run_blind_unseal_v8.py"
        self.contract_path = self.directory / "implementation-contract.json"
        runner_sha = sha256_bytes(self.runner_path.read_bytes())
        self.contract = json.loads((ROOT / "contracts" / "blind_runtime_unseal_v8.json").read_text(encoding="utf-8"))
        self.contract["status"] = "FINAL_HELD_UNSEAL_REVIEW_REQUIRED"
        self.contract["job_template"] = {"path": "contracts/blind_runtime_unseal_job_v8_template.json", "sha256": sha256_bytes(self.template_path.read_bytes())}
        ...
        self.reg = registration(self.bjobs, self.global_bjobs, self.template_path.read_bytes(), self.contract_path.read_bytes())
        ...
        self.tool = sha256_bytes(("src/data/run_blind_unseal_v8.py:" + runner_sha + "\n").encode("utf-8"))
    ...
    def valid_audit(self):
        return validate_scheduler_audit(self.audit, self.reg, self.raw_registration, self.bjobs, self.bhist, self.bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs)

    def test_real_lsf_9_1_resume_fixture_and_exact_receipt_pass(self):
        self.assertTrue(validate_registration(self.reg, self.bjobs, "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs, verify_git_blob=False))
    ...
    def test_no_trailing_delimiter_and_extra_receipt_field_fail(self):
        ...
        with self.assertRaises(ValidationError): validate_registration(self.reg, self.bjobs.rstrip() + b"^\n", "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs, verify_git_blob=False)
        changed = copy.deepcopy(self.reg); changed["unexpected"] = True
        with self.assertRaises(ValidationError): validate_registration(changed, self.bjobs, "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs, verify_git_blob=False)
    ...
    def test_global_duplicate_identity_rejected_foreign_held_allowed(self):
        ...
        with self.assertRaises(ValidationError): validate_registration(changed, self.bjobs, "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), duplicate, verify_git_blob=False)
        ...
        self.assertTrue(validate_registration(changed, self.bjobs, "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), foreign, verify_git_blob=False))
    ...
    def test_bjobs_mutations_fail(self):
        for changed in (raw_bjobs(state="PEND"), raw_bjobs(command="evil --candidate"), self.bjobs.replace(b"jiangchuanc", b"other-user")):
            ...
            with self.assertRaises(ValidationError): validate_registration(changed_reg, changed, "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs, verify_git_blob=False)
    ...
    def test_history_missing_cont_duplicate_resume_and_terminal_mismatch_fail(self):
        ...
        for bhist, bacct in cases:
            ...
            with self.assertRaises(ValidationError): validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, bhist, bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs)
    ...
    def test_clock_skew_allowed_wrapper_order_and_window_enforced(self):
        ...
        self.assertTrue(validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, skewed, self.bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs))
        ...
        with self.assertRaises(ValidationError): validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, late, late_bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs)
    ...
    def test_foreign_history_ignored_target_forbidden_and_artifact_leak_fail(self):
        ...
        self.assertTrue(validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, foreign_bhist, foreign_bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs))
        ...
        with self.assertRaises(ValidationError): validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, bad, self.bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs)
    ...
    def test_audit_global_hash_and_release_drift_fail(self):
        ...
        with self.assertRaises(ValidationError): validate_scheduler_audit(item, self.reg, self.raw_registration, self.bjobs, self.bhist, self.bacct, self.stdout, self.stderr, str(self.directory), "12345", str(self.template_path), str(self.contract_path), str(self.runner_path), self.global_bjobs)
