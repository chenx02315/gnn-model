#!/usr/bin/env python3
"""Fail-closed, no-I/O construction of the v11 held-job lifecycle."""
...
from validate_blind_unseal_registration_v11 import (EXPECTED_EXTERNAL_CONTROL_MANIFEST,
    FIELDS as REGISTRATION_FIELDS)
from validate_blind_unseal_registration_v11 import ValidationError, validate_registration
...
def _template_shape(value):
    ...
    _require(value["schema_version"] == "blind-runtime-unseal-job-v11" and value["status"] == "FINAL_HELD_JOB_TEMPLATE", "TEMPLATE_STATE")
...
def _contract_shape(value):
    ...
    _require(value["schema_version"] == "blind-runtime-unseal-v11" and value["status"] == "FINAL_HELD_UNSEAL_REVIEW_REQUIRED", "CONTRACT_STATE")
...
def materialize_final_template(draft_template, held_job, circuit_roots):
    ...
    result = {"schema_version": "blind-runtime-unseal-job-v11", "status": "FINAL_HELD_JOB_TEMPLATE",
...
def finalize_contract(draft_contract, final_template_bytes, implementation_freeze_commit, implementation_artifact_sha256):
    ...
    result["purpose"] = "v11 sealed BLIND runtime audit with pre-staged clean Git checkout and exact absolute CWD bindings."
    result["job_template"] = {"path": "contracts/blind_runtime_unseal_job_v11_template.json", "sha256": sha256_bytes(final_template_bytes)}
...
def _job_spec_shape(spec, template, receipt, contract, receipt_bytes, final_template_bytes, final_contract_bytes):
    ...
    _require(spec["schema_version"] == "blind-runtime-unseal-job-v11" and spec["status"] == "REGISTERED_HELD", "JOB_SPEC_STATE")
