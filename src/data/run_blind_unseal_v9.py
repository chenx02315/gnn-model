#!/usr/bin/env python3
"""Sealed v9 BLIND runtime auditor; stdlib-only, operational one-shot control."""
...
CONTROL_ARTIFACTS = frozenset((
    "contracts/blind_runtime_unseal_v9.json",
    "contracts/blind_runtime_unseal_job_v9_template.json",
    "contracts/blind_runtime_unseal_job_v9.json",
    "contracts/blind_runtime_unseal_registration_v9.json",
    "contracts/blind_unseal_independent_review_v9.json",
    ...
    "src/data/build_blind_unseal_lifecycle_v9.py",
    "src/data/run_blind_unseal_v9.py",
    "src/data/validate_blind_unseal_registration_v9.py",
    "src/data/validate_blind_unseal_scheduler_audit_v9.py",
))
...
EXPECTED_EXTERNAL_CONTROL_MANIFEST = "/temp/jiangchuanc/blind_runtime_unseal_v9_control_manifest.json"
...
def _require_source_guard(guard):
    ...
    if marker!={"schema_version":"blind-runtime-unseal-consumed-v9","status":"CONSUMED","contract_sha256":contract_sha,"tool_set_sha256":tool_sha}: raise Refusal("SOURCE_ACCESS_BEFORE_CONSUMED")
...
def atomic(path,payload):
    fd,tmp=tempfile.mkstemp(prefix=".v9-",dir=os.path.dirname(path))
    try:
        with os.fdopen(fd,"wb") as f: f.write(payload); f.flush(); os.fsync(f.fileno())
...
def verify_release(contract,contract_sha,tool_sha,out):
    ...
    if side != [dig,"receipt.json"] or consumed!={"schema_version":"blind-runtime-unseal-consumed-v9","status":"CONSUMED","contract_sha256":contract_sha,"tool_set_sha256":tool_sha} or released!={"schema_version":"blind-runtime-unseal-release-v9","status":"RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING","contract_sha256":contract_sha,"receipt_sha256":dig}: return False
...
def verify_receipt(contract,contract_sha,tool_sha,receipt):
    ...
    if set(receipt)!=envelope or receipt.get("schema_version")!="blind-runtime-unseal-receipt-v9" or receipt.get("formal_runtime_membership_sha256")!=contract["scope"].get("formal_runtime_membership_sha256") or receipt.get("method_registry_sha256")!=contract["scope"].get("method_registry_sha256") or receipt.get("tool_set_sha256")!=tool_sha or receipt.get("contract_sha256")!=contract_sha: return False
...
def verify_external_control_manifest(root, reviewed_commit, manifest_path, expected_digest):
    ...
    if set(manifest)!={"schema_version","bundle_root","freeze_commit","files"} or manifest.get("schema_version")!="blind-runtime-unseal-control-manifest-v9" or manifest.get("bundle_root")!=root or manifest.get("freeze_commit")!=reviewed_commit:
...
def validate_bundle(root):
    contract_rel="contracts/blind_runtime_unseal_v9.json"; contract=control_read_json(root,contract_rel); csha=control_sha_file(root,contract_rel)
    ...
    if job.get("schema_version")!="blind-runtime-unseal-job-v9" or job.get("status")!="REGISTERED_HELD" or job.get("contract_sha256")!=csha: raise Refusal("JOB_CONTRACT_BINDING")
    ...
    if job.get("status")!="REGISTERED_HELD" or set(registration)!=set(required) or registration.get("schema_version")!="blind-runtime-unseal-registration-v9" or registration.get("status")!="REGISTERED_HELD" or template.get("initial_scheduler_state")!="PSUSP" or any(template.get(k) is not False for k in ("array_allowed","retry_allowed","requeue_allowed","rerun_allowed")):
    ...
    if (registration.get("contract_sha256")!=csha or registration.get("protocol_sha256")!=contract["protocol"]["sha256"] or registration.get("gate_snapshot_sha256")!=contract["gate_snapshot"]["sha256"] or registration.get("job_template_sha256")!=template_sha or registration.get("runner_sha256")!=contract.get("implementation",{}).get("artifact_sha256",{}).get("src/data/run_blind_unseal_v9.py")):
...
def write_release(contract,csha,tool,out,receipt):
    ...
    exclusive(os.path.join(out,"RELEASED"),json_bytes({"schema_version":"blind-runtime-unseal-release-v9","status":"RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING","contract_sha256":csha,"receipt_sha256":digest}))
def run(root):
    ...
    os.mkdir(out,0o700); fsync_dir(os.path.dirname(out)); tool=sha_lines(k+":"+v for k,v in artifacts.items()); exclusive(os.path.join(out,"CONSUMED"),json_bytes({"schema_version":"blind-runtime-unseal-consumed-v9","status":"CONSUMED","contract_sha256":csha,"tool_set_sha256":tool}))
    ...
    try:
        ...
        receipt={"schema_version":"blind-runtime-unseal-receipt-v9","status":"PASS","formal_runtime_membership_sha256":contract["scope"]["formal_runtime_membership_sha256"],"method_registry_sha256":contract["scope"]["method_registry_sha256"],"tool_set_sha256":tool,"contract_sha256":csha,"circuits":rows,"failure_code":None}; code=0
    except Exception as error:
        failure_code="R06_R07_FAILED" if isinstance(error,ValueError) and str(error)=="R06_R07_FAILED" else "SEALED_AUDIT_FAILED"
        receipt={"schema_version":"blind-runtime-unseal-receipt-v9","status":"FAIL","formal_runtime_membership_sha256":contract["scope"]["formal_runtime_membership_sha256"],"method_registry_sha256":contract["scope"]["method_registry_sha256"],"tool_set_sha256":tool,"contract_sha256":csha,"failure_code":failure_code,"circuits":[]}; code=1
    finally:
        _ACTIVE_SOURCE_CAPABILITY=None
