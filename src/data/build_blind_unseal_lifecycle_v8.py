#!/usr/bin/env python3
"""Fail-closed, no-I/O construction of the v8 held-job lifecycle."""
...
from validate_blind_unseal_registration_v8 import FIELDS as REGISTRATION_FIELDS
from validate_blind_unseal_registration_v8 import ValidationError, validate_registration
...
def _contract_shape(value):
    ...
    _require(value["schema_version"] == "blind-runtime-unseal-v8" and value["status"] == "FINAL_HELD_UNSEAL_REVIEW_REQUIRED", "CONTRACT_STATE")
...
def materialize_final_template(draft_template, held_job, circuit_roots):
    ...
    result = {"schema_version": "blind-runtime-unseal-job-v8", "status": "FINAL_HELD_JOB_TEMPLATE",
...
def finalize_contract(draft_contract, final_template_bytes, implementation_freeze_commit, implementation_artifact_sha256):
    ...
    result["purpose"] = "v8 sealed BLIND runtime audit; runner release requires a separately captured scheduler audit."
    result["job_template"] = {"path": "contracts/blind_runtime_unseal_job_v8_template.json", "sha256": sha256_bytes(final_template_bytes)}
    ...
    result["lifecycle"] = {"implementation_freeze_commit": implementation_freeze_commit, "final_template_sha256": sha256_bytes(final_template_bytes), "artifact_blob_equals_worktree_required": True, "finalization_rule": "Stable contract binds only final template and implementation freeze. Security boundary is CLI_ONLY_TRUSTED_PROCESS; same-user Python imports and direct filesystem reads are outside scope.", "required_order": ["implementation_freeze_commit", "bsub_H", "raw_bjobs_capture", "final_template", "final_contract", "registration_receipt", "final_job_spec", "independent_review", "clean_commit", "bresume"]}
    _no_null_or_pending(result); _contract_shape(result)
    return result


def build_registration_receipt(final_template_bytes, final_contract_bytes, raw_bjobs_sha256, raw_global_bjobs_sha256, owner, submission_host, submit_time_raw):
    ...
    for d in (raw_bjobs_sha256, raw_global_bjobs_sha256): _digest(d, "RAW_CAPTURE_DIGEST")
    ...
    return {"schema_version": "blind-runtime-unseal-registration-v8", "status": "REGISTERED_HELD", "job_id": r["job_id"], "owner": owner, "submission_host": submission_host, "submit_time_raw": submit_time_raw, "reviewed_commit": r["git_commit"], "contract_sha256": sha256_bytes(final_contract_bytes), "protocol_sha256": contract["protocol"]["sha256"], "gate_snapshot_sha256": contract["gate_snapshot"]["sha256"], "runner_sha256": contract["implementation"]["artifact_sha256"]["src/data/run_blind_unseal_v8.py"], "job_template_sha256": sha256_bytes(final_template_bytes), "normalized_argv": argv, "normalized_argv_sha256": sha256_bytes(json.dumps(argv, ensure_ascii=True, separators=(",", ":")).encode("utf-8")), "queue": r["queue"], "resource_request": r["resource_request"], "cwd": r["cwd"], "stdout_path": r["stdout_path"], "stderr_path": r["stderr_path"], "raw_bjobs_capture_sha256": raw_bjobs_sha256, "raw_global_bjobs_capture_sha256": raw_global_bjobs_sha256}


def build_final_job_spec(receipt_bytes, final_template_bytes, final_contract_bytes, output_root, registration_receipt_path="contracts/blind_runtime_unseal_registration_v8.json"):
...
def _job_spec_shape(spec, template, receipt, contract, receipt_bytes, final_template_bytes, final_contract_bytes):
    ...
    _require(spec["schema_version"] == "blind-runtime-unseal-job-v8" and spec["status"] == "REGISTERED_HELD", "JOB_SPEC_STATE")
...
def build_independent_review(receipt_bytes, job_spec_bytes, final_contract_bytes, raw_bjobs, raw_global_bjobs, expected_job_id, job_template_path, implementation_contract_path, runner_path):
    ...
    try: validate_registration(receipt, raw_bjobs, expected_job_id, job_template_path, implementation_contract_path, runner_path, raw_global_bjobs, verify_git_blob=True)
...
def validate_one_way_lifecycle(final_template_bytes, final_contract_bytes, receipt_bytes, job_spec_bytes, review_bytes):
    ...
    for key in ("protocol_sha256", "gate_snapshot_sha256", "normalized_argv", "raw_bjobs_capture_sha256", "raw_global_bjobs_capture_sha256", "queue", "resource_request", "cwd", "stdout_path", "stderr_path"):
