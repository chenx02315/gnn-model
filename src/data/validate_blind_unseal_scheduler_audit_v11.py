#!/usr/bin/env python3
"""Fail-closed v11 post-run scheduler audit from A-side raw evidence files."""
...
def _verify_four_artifacts(directory, contract, contract_sha):
    ...
    _require(consumed == {"schema_version": "blind-runtime-unseal-consumed-v11", "status": "CONSUMED", "contract_sha256": contract_sha, "tool_set_sha256": tool_sha}, "CONSUMED_SCHEMA")
    _require(released == {"schema_version": "blind-runtime-unseal-release-v11", "status": "RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING", "contract_sha256": contract_sha, "receipt_sha256": receipt_sha}, "RELEASED_SCHEMA")
    pass_envelope = {"schema_version", "status", "formal_runtime_membership_sha256", "method_registry_sha256", "tool_set_sha256", "contract_sha256", "circuits", "failure_code"}
    fail_envelope = {"schema_version", "status", "contract_sha256", "tool_set_sha256", "failure_stage", "failure_code", "circuits"}
    _require(receipt.get("schema_version") == "blind-runtime-unseal-receipt-v11" and receipt.get("status") in ("PASS", "FAIL") and receipt.get("contract_sha256") == contract_sha and receipt.get("tool_set_sha256") == tool_sha, "RUNNER_RECEIPT_BINDING")
    if receipt["status"] == "PASS":
        _require(set(receipt) == pass_envelope and receipt.get("formal_runtime_membership_sha256") == contract.get("scope", {}).get("formal_runtime_membership_sha256") and receipt.get("method_registry_sha256") == contract.get("scope", {}).get("method_registry_sha256"), "RUNNER_PASS_SCHEMA")
        ...
        _require(set(receipt) == fail_envelope and receipt.get("circuits") == [] and taxonomy.get(receipt.get("failure_stage")) == receipt.get("failure_code"), "RUNNER_FAIL_ENVELOPE")
...
def validate_scheduler_audit(receipt, registration, raw_registration, raw_bjobs, raw_bhist, raw_bacct, stdout_raw, stderr_raw, artifact_directory, expected_job_id, job_template_path, implementation_contract_path, runner_path, raw_global_bjobs, raw_bjobs_al, external_control_manifest):
    ...
    _require(receipt["schema_version"] == "blind-runtime-unseal-scheduler-audit-v11", "AUDIT_SCHEMA_VERSION")
...
def main(argv=None):
    ...
    try:
        ...
        print("BLIND_UNSEAL_SCHEDULER_AUDIT_v11=FAIL:%s" % error)
        return 1
    print("BLIND_UNSEAL_SCHEDULER_AUDIT_v11=PASS")
    return 0
