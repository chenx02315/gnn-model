#!/usr/bin/env python3
"""Fail-closed v11 registration validator using the raw A-side bjobs capture."""
...
def verify_staged_bundle(bundle_root, freeze_commit, external_manifest_bytes):
    ...
    _require(manifest["schema_version"] == "blind-runtime-unseal-control-manifest-v11", "EXTERNAL_MANIFEST_SCHEMA")
...
def _repo_root_for_runner(runner_path):
    ...
    expected = os.path.join(root, "src", "data", "run_blind_unseal_v11_final.py")
...
def validate_registration(receipt, raw_bjobs, expected_job_id, job_template_path, implementation_contract_path, runner_path, raw_global_bjobs, raw_bjobs_al=None, external_control_manifest=None):
    ...
    _require(receipt["schema_version"] == "blind-runtime-unseal-registration-v11", "SCHEMA_VERSION")
    ...
    _require(contract.get("schema_version") == "blind-runtime-unseal-v11", "IMPLEMENTATION_CONTRACT_SCHEMA")
    ...
    _require(contract.get("implementation", {}).get("artifact_sha256", {}).get("src/data/run_blind_unseal_v11_final.py") == receipt["runner_sha256"], "RUNNER_CONTRACT_BINDING")
...
def main(argv=None):
    ...
    try:
        ...
        print("BLIND_UNSEAL_REGISTRATION_v11=FAIL:%s" % error)
        return 1
    print("BLIND_UNSEAL_REGISTRATION_v11=PASS")
    return 0
