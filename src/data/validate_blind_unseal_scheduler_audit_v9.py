#!/usr/bin/env python3
"""Fail-closed v9 post-run scheduler audit from A-side raw evidence files."""
...
def _verify_four_artifacts(directory, contract, contract_sha):
    ...
    _require(consumed == {"schema_version": "blind-runtime-unseal-consumed-v9", "status": "CONSUMED", "contract_sha256": contract_sha, "tool_set_sha256": tool_sha}, "CONSUMED_SCHEMA")
    _require(released == {"schema_version": "blind-runtime-unseal-release-v9", "status": "RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING", "contract_sha256": contract_sha, "receipt_sha256": receipt_sha}, "RELEASED_SCHEMA")
...
def validate_scheduler_audit(receipt, registration, raw_registration, raw_bjobs, raw_bhist, raw_bacct, stdout_raw, stderr_raw, artifact_directory, expected_job_id, job_template_path, implementation_contract_path, runner_path, raw_global_bjobs, raw_bjobs_al, external_control_manifest):
    ...
    _require(receipt["schema_version"] == "blind-runtime-unseal-scheduler-audit-v9", "AUDIT_SCHEMA_VERSION")
...
def main(argv=None):
    ...
    try:
        ...
        print("BLIND_UNSEAL_SCHEDULER_AUDIT_v9=FAIL:%s" % error)
        return 1
    print("BLIND_UNSEAL_SCHEDULER_AUDIT_v9=PASS")
    return 0
