#!/usr/bin/env python3
"""Sealed v11 BLIND runtime auditor; stdlib-only, operational one-shot control."""
...
CONTROL_ARTIFACTS = frozenset((
    "contracts/blind_runtime_unseal_v11.json",
    "contracts/blind_runtime_unseal_job_v11_template.json",
    "contracts/blind_runtime_unseal_job_v11.json",
    "contracts/blind_runtime_unseal_registration_v11.json",
    "contracts/blind_unseal_independent_review_v11.json",
...
EXPECTED_BUNDLE_ROOT = "/temp/jiangchuanc/blind_runtime_unseal_v11_bundle"
EXPECTED_EXTERNAL_CONTROL_MANIFEST = "/temp/jiangchuanc/blind_runtime_unseal_v11_control_manifest.json"
EXPECTED_COMMAND_ARGV = ["python3", EXPECTED_BUNDLE_ROOT + "/src/data/run_blind_unseal_v11_final.py", "--bundle-root", EXPECTED_BUNDLE_ROOT]
...
class StageFailure(Exception):
    def __init__(self, stage, code):
        super(StageFailure, self).__init__(stage + ":" + code)
        self.stage = stage
        self.code = code
...
def _require_source_guard(guard):
    ...
    if marker!={"schema_version":"blind-runtime-unseal-consumed-v11","status":"CONSUMED","contract_sha256":contract_sha,"tool_set_sha256":tool_sha}: raise Refusal("SOURCE_ACCESS_BEFORE_CONSUMED")
...
def _audit_circuit(entry, measurement_files, guard):
    _require_source_guard(guard)
    current_stage="SOURCE_INVENTORY"
    try:
        ...
        current_stage="MEASUREMENT_PARSE"
        ...
        current_stage="JOIN_CLASSIFICATION"
        ...
        raise StageFailure(current_stage, FAILURE_TAXONOMY[current_stage])
...
def atomic(path,payload):
    fd,tmp=tempfile.mkstemp(prefix=".v11-",dir=os.path.dirname(path))
    try:
        with os.fdopen(fd,"wb") as f: f.write(payload); f.flush(); os.fsync(f.fileno())
...
def verify_release(contract,contract_sha,tool_sha,out):
    ...
    if side != [dig,"receipt.json"] or consumed!={"schema_version":"blind-runtime-unseal-consumed-v11","status":"CONSUMED","contract_sha256":contract_sha,"tool_set_sha256":tool_sha} or released!={"schema_version":"blind-runtime-unseal-release-v11","status":"RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING","contract_sha256":contract_sha,"receipt_sha256":dig}: return False
...
def verify_receipt(contract,contract_sha,tool_sha,receipt):
    ...
    if receipt.get("status")=="FAIL":
        failure_fields={"schema_version","status","contract_sha256","tool_set_sha256","failure_stage","failure_code","circuits"}
        return set(receipt)==failure_fields and receipt.get("schema_version")=="blind-runtime-unseal-receipt-v11" and receipt.get("contract_sha256")==contract_sha and receipt.get("tool_set_sha256")==tool_sha and receipt.get("circuits")==[] and FAILURE_TAXONOMY.get(receipt.get("failure_stage"))==receipt.get("failure_code")
    ...
    if set(receipt)!=envelope or receipt.get("schema_version")!="blind-runtime-unseal-receipt-v11" or receipt.get("formal_runtime_membership_sha256")!=contract["scope"].get("formal_runtime_membership_sha256") or receipt.get("method_registry_sha256")!=contract["scope"].get("method_registry_sha256") or receipt.get("tool_set_sha256")!=tool_sha or receipt.get("contract_sha256")!=contract_sha: return False
...
def verify_external_control_manifest(root, reviewed_commit, manifest_path, expected_digest):
    ...
    if set(manifest)!={"schema_version","bundle_root","freeze_commit","files"} or manifest.get("schema_version")!="blind-runtime-unseal-control-manifest-v11" or manifest.get("bundle_root")!=root or manifest.get("freeze_commit")!=reviewed_commit:
...
def validate_bundle(root):
    contract_rel="contracts/blind_runtime_unseal_v11.json"; contract=control_read_json(root,contract_rel); csha=control_sha_file(root,contract_rel)
    ...
    if job.get("schema_version")!="blind-runtime-unseal-job-v11" or job.get("status")!="REGISTERED_HELD" or job.get("contract_sha256")!=csha: raise Refusal("JOB_CONTRACT_BINDING")
    ...
    if job.get("status")!="REGISTERED_HELD" or set(registration)!=set(required) or registration.get("schema_version")!="blind-runtime-unseal-registration-v11" or registration.get("status")!="REGISTERED_HELD" or template.get("initial_scheduler_state")!="PSUSP" or any(template.get(k) is not False for k in ("array_allowed","retry_allowed","requeue_allowed","rerun_allowed")):
    ...
    if (registration.get("contract_sha256")!=csha or registration.get("protocol_sha256")!=contract["protocol"]["sha256"] or registration.get("gate_snapshot_sha256")!=contract["gate_snapshot"]["sha256"] or registration.get("job_template_sha256")!=template_sha or registration.get("runner_sha256")!=contract.get("implementation",{}).get("artifact_sha256",{}).get("src/data/run_blind_unseal_v11_final.py")):
...
def write_release(contract,csha,tool,out,receipt):
    ...
    exclusive(os.path.join(out,"RELEASED"),json_bytes({"schema_version":"blind-runtime-unseal-release-v11","status":"RELEASED_RUNNER_VERIFIED_SCHEDULER_AUDIT_PENDING","contract_sha256":csha,"receipt_sha256":digest}))
def run(root):
    ...
    os.mkdir(out,0o700); fsync_dir(os.path.dirname(out)); tool=sha_lines(k+":"+v for k,v in artifacts.items()); exclusive(os.path.join(out,"CONSUMED"),json_bytes({"schema_version":"blind-runtime-unseal-consumed-v11","status":"CONSUMED","contract_sha256":csha,"tool_set_sha256":tool}))
    ...
    try:
        ...
        if any(x["missing_runtime_join_count"] or x["ambiguous_runtime_join_count"] or x["coverage_rate"]!=1.0 or x["action_coverage_summary"]["all_unique_action_count"]!=x["action_coverage_summary"]["eligible_action_count"] for x in rows): raise StageFailure("COVERAGE_GATE",FAILURE_TAXONOMY["COVERAGE_GATE"])
        receipt={"schema_version":"blind-runtime-unseal-receipt-v11","status":"PASS","formal_runtime_membership_sha256":contract["scope"]["formal_runtime_membership_sha256"],"method_registry_sha256":contract["scope"]["method_registry_sha256"],"tool_set_sha256":tool,"contract_sha256":csha,"circuits":rows,"failure_code":None}; code=0
    except StageFailure as error:
        receipt={"schema_version":"blind-runtime-unseal-receipt-v11","status":"FAIL","contract_sha256":csha,"tool_set_sha256":tool,"failure_stage":error.stage,"failure_code":error.code,"circuits":[]}; code=1
    except Exception:
        receipt={"schema_version":"blind-runtime-unseal-receipt-v11","status":"FAIL","contract_sha256":csha,"tool_set_sha256":tool,"failure_stage":"INTERNAL_AUDIT","failure_code":FAILURE_TAXONOMY["INTERNAL_AUDIT"],"circuits":[]}; code=1
    finally:
        _ACTIVE_SOURCE_CAPABILITY=None
    if not verify_receipt(contract,csha,tool,receipt):
        receipt={"schema_version":"blind-runtime-unseal-receipt-v11","status":"FAIL","contract_sha256":csha,"tool_set_sha256":tool,"failure_stage":"RELEASE_INTEGRITY","failure_code":FAILURE_TAXONOMY["RELEASE_INTEGRITY"],"circuits":[]}; code=1
