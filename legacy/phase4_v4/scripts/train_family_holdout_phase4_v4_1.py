...
def load_inputs(dataset: Path, graph_dir: Path, family_map: Path):
    ...
    if not (
        (safe_anchor_rows.feasible_at_d95 == 1)
        & (safe_anchor_rows.detected_faults >= safe_anchor_rows.d95)
    ).all():
        raise RuntimeError("Coordinate-fixed FullScan-F4 anchor contract is not D95-feasible")
